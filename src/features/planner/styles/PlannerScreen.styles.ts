import { StyleSheet } from "react-native";

import { COLORS } from "../../../constants/colors";
import { SPACING } from "../../../constants/spacing";

export const plannerStyles = StyleSheet.create({
  scrollContent: {
    paddingHorizontal: SPACING.lg,
    paddingTop: 18,
  },
  hero: {
    backgroundColor: COLORS.surfaceContainerLowest,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    padding: 16,
    marginBottom: 18,
  },
  heroHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: 14,
  },
  eyebrow: {
    color: COLORS.primary,
    fontSize: 12,
    fontWeight: "800",
    marginBottom: 3,
  },
  heroTitle: {
    color: COLORS.onSurface,
    fontSize: 24,
    lineHeight: 30,
    fontWeight: "900",
  },
  heroStats: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 8,
  },
  miniStat: {
    minWidth: "23%",
    flex: 1,
    backgroundColor: COLORS.surfaceContainer,
    borderRadius: 8,
    paddingVertical: 10,
    paddingHorizontal: 10,
  },
  miniStatValue: {
    color: COLORS.onSurface,
    fontSize: 18,
    fontWeight: "900",
  },
  miniStatLabel: {
    color: COLORS.onSurfaceVariant,
    fontSize: 11,
    fontWeight: "700",
    marginTop: 2,
  },
  workspaceSwitch: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 8,
    marginTop: 14,
  },
  section: {
    marginBottom: 20,
  },
  sectionHeader: {
    marginBottom: 12,
  },
  sectionTitle: {
    fontSize: 20,
    lineHeight: 26,
    fontWeight: "900",
    color: COLORS.onSurface,
  },
  sectionSubtitle: {
    marginTop: 4,
    color: COLORS.onSurfaceVariant,
    fontSize: 13,
    lineHeight: 19,
  },
  collapsiblePanel: {
    marginBottom: 20,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    borderRadius: 8,
    backgroundColor: COLORS.surfaceContainerLowest,
    overflow: "hidden",
  },
  collapsiblePanelHeader: {
    minHeight: 72,
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
    paddingHorizontal: 16,
    paddingVertical: 14,
  },
  collapsiblePanelTitleBlock: {
    flex: 1,
  },
  collapsiblePanelTitle: {
    color: COLORS.onSurface,
    fontSize: 18,
    lineHeight: 24,
    fontWeight: "900",
  },
  collapsiblePanelSubtitle: {
    marginTop: 4,
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    lineHeight: 18,
  },
  collapsiblePanelToggle: {
    width: 38,
    height: 38,
    borderRadius: 8,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: COLORS.surfaceContainer,
  },
  collapsiblePanelContent: {
    borderTopWidth: 1,
    borderTopColor: COLORS.outlineVariant,
    padding: 16,
  },
  formGrid: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 10,
  },
  field: {
    flex: 1,
    minWidth: 144,
  },
  fieldLabel: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    fontWeight: "800",
    marginBottom: 5,
  },
  input: {
    minHeight: 46,
    backgroundColor: COLORS.surfaceContainerLowest,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    borderRadius: 8,
    paddingHorizontal: 12,
    paddingVertical: 9,
    color: COLORS.onSurface,
    fontSize: 14,
    marginBottom: 10,
  },
  quantityPanel: {
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    borderRadius: 8,
    backgroundColor: COLORS.surfaceContainer,
    padding: 12,
    marginBottom: 12,
  },
  quantityLabel: {
    color: COLORS.onSurface,
    fontSize: 13,
    fontWeight: "900",
    marginBottom: 8,
  },
  quantityRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  },
  quantityInput: {
    flex: 1,
    marginBottom: 0,
  },
  quantityUnit: {
    minWidth: 44,
    color: COLORS.onSurface,
    fontSize: 14,
    fontWeight: "900",
  },
  quantityHint: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    lineHeight: 17,
    marginTop: 8,
  },
  goalInput: {
    minHeight: 52,
    backgroundColor: COLORS.surfaceContainerLowest,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    borderRadius: 8,
    paddingHorizontal: 14,
    paddingVertical: 10,
    color: COLORS.onSurface,
    fontSize: 20,
    fontWeight: "900",
    marginTop: 12,
    marginBottom: 12,
  },
  textArea: {
    minHeight: 76,
    textAlignVertical: "top",
  },
  inputWithButton: {
    flexDirection: "row",
    gap: 10,
    alignItems: "center",
  },
  inputGrow: {
    flex: 1,
  },
  squareButton: {
    width: 46,
    height: 46,
    borderRadius: 8,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: COLORS.primary,
    marginBottom: 10,
  },
  segmentRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 8,
    marginBottom: 12,
  },
  recommendationFilterScroll: {
    marginBottom: 12,
  },
  recommendationFilterContent: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
    paddingRight: SPACING.lg,
  },
  recommendationChip: {
    minHeight: 38,
    borderRadius: 999,
    paddingHorizontal: 15,
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    position: "relative",
  },
  recommendationChipActive: {
    backgroundColor: COLORS.primary,
    borderColor: COLORS.primary,
  },
  recommendationChipText: {
    color: COLORS.onSurfaceVariant,
    fontWeight: "900",
    fontSize: 13,
  },
  recommendationChipTextActive: {
    color: COLORS.onPrimary,
  },
  recommendationBadgeDot: {
    position: "absolute",
    top: -2,
    right: -2,
    width: 10,
    height: 10,
    borderRadius: 5,
    backgroundColor: COLORS.tertiary,
    borderWidth: 2,
    borderColor: COLORS.surfaceContainerLowest,
  },
  chip: {
    minHeight: 36,
    borderRadius: 999,
    paddingHorizontal: 14,
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainer,
  },
  chipActive: {
    backgroundColor: COLORS.primary,
    borderColor: COLORS.primary,
  },
  chipText: {
    color: COLORS.onSurfaceVariant,
    fontWeight: "800",
    fontSize: 13,
  },
  chipTextActive: {
    color: COLORS.onPrimary,
  },
  actionRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 10,
    marginTop: 2,
    marginBottom: 10,
  },
  button: {
    minHeight: 42,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 6,
    borderRadius: 8,
    paddingHorizontal: 14,
    backgroundColor: COLORS.primary,
  },
  secondaryButton: {
    backgroundColor: COLORS.surfaceContainerLowest,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
  },
  disabledButton: {
    opacity: 0.55,
  },
  buttonText: {
    fontSize: 13,
    fontWeight: "800",
    color: COLORS.onPrimary,
  },
  secondaryButtonText: {
    color: COLORS.primary,
  },
  inlineNotice: {
    flexDirection: "row",
    alignItems: "flex-start",
    gap: 8,
    borderRadius: 8,
    padding: 12,
    backgroundColor: "#e7f5ee",
    borderWidth: 1,
    borderColor: "#b7dccb",
  },
  inlineNoticeText: {
    flex: 1,
    color: COLORS.onSurface,
    fontSize: 13,
    lineHeight: 19,
    fontWeight: "700",
  },
  bucket: {
    marginBottom: 12,
  },
  bucketHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: 8,
  },
  bucketTitle: {
    color: COLORS.onSurface,
    fontSize: 15,
    fontWeight: "900",
  },
  bucketCount: {
    color: COLORS.primary,
    fontWeight: "900",
  },
  bucketWarning: {
    color: COLORS.tertiary,
  },
  foodStrip: {
    gap: 8,
  },
  foodCard: {
    width: 236,
    minHeight: 260,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    overflow: "hidden",
  },
  foodCardWarning: {
    borderColor: COLORS.tertiary,
  },
  foodCardImage: {
    width: "100%",
    height: 108,
    backgroundColor: COLORS.surfaceContainer,
  },
  foodImagePlaceholder: {
    alignItems: "center",
    justifyContent: "center",
  },
  foodCardBody: {
    flex: 1,
    padding: 11,
    justifyContent: "space-between",
  },
  foodPill: {
    width: 172,
    minHeight: 118,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    padding: 10,
    justifyContent: "space-between",
  },
  foodName: {
    color: COLORS.onSurface,
    fontWeight: "900",
    fontSize: 14,
    marginBottom: 4,
  },
  foodMeta: {
    color: COLORS.onSurfaceVariant,
    fontWeight: "700",
    fontSize: 12,
    lineHeight: 17,
  },
  foodNutrientRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 6,
    marginTop: 9,
  },
  foodKcal: {
    color: COLORS.primary,
    fontSize: 12,
    fontWeight: "900",
    backgroundColor: COLORS.surfaceContainer,
    paddingHorizontal: 7,
    paddingVertical: 3,
    borderRadius: 4,
  },
  foodMacroText: {
    color: COLORS.onSurfaceVariant,
    fontSize: 11,
    fontWeight: "800",
    backgroundColor: COLORS.surfaceContainer,
    paddingHorizontal: 7,
    paddingVertical: 3,
    borderRadius: 4,
  },
  foodPlanButton: {
    minHeight: 28,
    marginTop: 8,
    borderRadius: 6,
    backgroundColor: COLORS.primary,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 4,
  },
  foodPlanButtonText: {
    color: COLORS.onPrimary,
    fontSize: 11,
    fontWeight: "900",
  },
  detailBackdrop: {
    flex: 1,
    backgroundColor: "rgba(0,0,0,0.36)",
    justifyContent: "center",
    paddingHorizontal: 18,
  },
  keyboardAvoidingView: {
    flex: 1,
  },
  detailSheet: {
    maxHeight: "86%",
    borderRadius: 8,
    backgroundColor: COLORS.surface,
    padding: 14,
  },
  scheduleSheet: {
    maxHeight: "90%",
  },
  scheduleSheetContent: {
    paddingBottom: 8,
  },
  goalSheet: {
    borderRadius: 8,
    backgroundColor: COLORS.surface,
    padding: 14,
  },
  detailHeader: {
    minHeight: 42,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    gap: 12,
    marginBottom: 10,
  },
  detailTitle: {
    flex: 1,
    color: COLORS.onSurface,
    fontSize: 20,
    lineHeight: 26,
    fontWeight: "900",
  },
  detailCloseButton: {
    minWidth: 58,
    height: 36,
    borderRadius: 18,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: COLORS.surfaceContainer,
  },
  detailCloseText: {
    color: COLORS.primary,
    fontSize: 12,
    fontWeight: "900",
  },
  foodDetailImage: {
    width: "100%",
    height: 190,
    borderRadius: 8,
    marginBottom: 12,
    backgroundColor: COLORS.surfaceContainer,
  },
  detailMetricGrid: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 8,
    marginBottom: 12,
  },
  detailMetric: {
    flex: 1,
    minWidth: "22%",
    minHeight: 58,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    justifyContent: "center",
    paddingHorizontal: 10,
  },
  detailMetricValue: {
    color: COLORS.primary,
    fontSize: 16,
    fontWeight: "900",
  },
  detailMetricLabel: {
    color: COLORS.onSurfaceVariant,
    fontSize: 11,
    fontWeight: "800",
    marginTop: 2,
  },
  detailInfoList: {
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    marginBottom: 12,
  },
  detailInfoRow: {
    minHeight: 42,
    flexDirection: "row",
    justifyContent: "space-between",
    gap: 12,
    paddingHorizontal: 12,
    paddingVertical: 9,
    borderBottomWidth: 1,
    borderBottomColor: COLORS.outlineVariant,
  },
  detailInfoLabel: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    fontWeight: "800",
  },
  detailInfoValue: {
    flex: 1,
    color: COLORS.onSurface,
    fontSize: 13,
    fontWeight: "800",
    textAlign: "right",
  },
  detailPrimaryButton: {
    minHeight: 44,
    borderRadius: 8,
    backgroundColor: COLORS.primary,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 6,
    marginBottom: 4,
  },
  detailPrimaryButtonText: {
    color: COLORS.onPrimary,
    fontSize: 13,
    fontWeight: "900",
  },
  seeAllButton: {
    minHeight: 42,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.primary,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: COLORS.surfaceContainerLowest,
  },
  seeAllButtonText: {
    color: COLORS.primary,
    fontSize: 13,
    fontWeight: "900",
  },
  confirmScheduleButton: {
    minHeight: 44,
    borderRadius: 8,
    backgroundColor: COLORS.primary,
    alignItems: "center",
    justifyContent: "center",
    paddingHorizontal: 14,
  },
  confirmScheduleButtonText: {
    color: COLORS.onPrimary,
    fontSize: 13,
    fontWeight: "900",
  },
  cancelScheduleButton: {
    minHeight: 44,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    alignItems: "center",
    justifyContent: "center",
    paddingHorizontal: 14,
  },
  cancelScheduleButtonText: {
    color: COLORS.primary,
    fontSize: 13,
    fontWeight: "900",
  },
  noticeBackdrop: {
    flex: 1,
    backgroundColor: "rgba(0,0,0,0.38)",
    justifyContent: "center",
    paddingHorizontal: 20,
  },
  noticeSheet: {
    borderRadius: 8,
    backgroundColor: COLORS.surface,
    padding: 18,
    borderWidth: 1,
    borderColor: COLORS.secondaryContainer,
  },
  noticeIconWrap: {
    width: 58,
    height: 58,
    borderRadius: 29,
    backgroundColor: COLORS.secondaryContainer,
    alignItems: "center",
    justifyContent: "center",
    marginBottom: 12,
  },
  noticeTitle: {
    color: COLORS.onSurface,
    fontSize: 21,
    lineHeight: 27,
    fontWeight: "900",
    marginBottom: 6,
  },
  noticeText: {
    color: COLORS.onSurfaceVariant,
    fontSize: 13,
    lineHeight: 20,
    fontWeight: "700",
    marginBottom: 12,
  },
  noticeList: {
    borderRadius: 8,
    backgroundColor: COLORS.surfaceContainerLowest,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    padding: 10,
    marginBottom: 12,
  },
  noticeItemRow: {
    minHeight: 30,
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  },
  noticeItemText: {
    flex: 1,
    color: COLORS.onSurface,
    fontSize: 13,
    fontWeight: "800",
  },
  noticePrimaryButton: {
    minHeight: 44,
    borderRadius: 8,
    backgroundColor: COLORS.primary,
    alignItems: "center",
    justifyContent: "center",
    paddingHorizontal: 14,
  },
  noticePrimaryButtonText: {
    color: COLORS.onPrimary,
    fontSize: 13,
    fontWeight: "900",
  },
  noticeSecondaryButton: {
    minHeight: 44,
    borderRadius: 8,
    backgroundColor: COLORS.secondaryContainer,
    alignItems: "center",
    justifyContent: "center",
    paddingHorizontal: 14,
  },
  noticeSecondaryButtonText: {
    color: COLORS.onSecondaryContainer,
    fontSize: 13,
    fontWeight: "900",
  },
  metricGrid: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 10,
  },
  metric: {
    width: "48%",
    minHeight: 72,
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    padding: 12,
    justifyContent: "center",
  },
  metricValue: {
    fontSize: 17,
    fontWeight: "900",
    color: COLORS.primary,
    marginBottom: 3,
  },
  metricLabel: {
    fontSize: 12,
    fontWeight: "700",
    color: COLORS.onSurfaceVariant,
  },
  extractionBox: {
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    padding: 12,
  },
  extractionTitle: {
    color: COLORS.onSurface,
    fontSize: 16,
    fontWeight: "900",
    marginBottom: 3,
  },
  extractionMeta: {
    color: COLORS.primary,
    fontSize: 12,
    fontWeight: "800",
    marginBottom: 8,
  },
  extractionIngredient: {
    color: COLORS.onSurfaceVariant,
    fontSize: 13,
    lineHeight: 19,
    flex: 1,
  },
  extractionHint: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    marginTop: 8,
    marginBottom: 4,
    fontStyle: "italic",
  },
  extractionHeader: {
    flexDirection: "row",
    alignItems: "flex-start",
    justifyContent: "space-between",
    gap: 8,
    marginBottom: 8,
  },
  extractionBadge: {
    backgroundColor: COLORS.primaryContainer,
    paddingHorizontal: 10,
    paddingVertical: 4,
    borderRadius: 12,
  },
  extractionBadgeText: {
    color: COLORS.onPrimaryContainer,
    fontSize: 11,
    fontWeight: "800",
  },
  extractionMetaRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 6,
    marginBottom: 10,
  },
  extractionChip: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
    backgroundColor: COLORS.surfaceContainer,
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 12,
  },
  extractionChipText: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    fontWeight: "700",
  },
  extractionDescription: {
    color: COLORS.onSurface,
    fontSize: 13,
    lineHeight: 19,
    marginBottom: 10,
  },
  extractionSection: {
    marginTop: 10,
    paddingTop: 10,
    borderTopWidth: 1,
    borderTopColor: COLORS.outlineVariant + "60",
  },
  extractionSectionTitle: {
    color: COLORS.onSurface,
    fontSize: 14,
    fontWeight: "800",
    marginBottom: 6,
  },
  extractionRow: {
    flexDirection: "row",
    alignItems: "flex-start",
    gap: 6,
    marginBottom: 4,
  },
  extractionStepNumber: {
    color: COLORS.primary,
    fontSize: 13,
    fontWeight: "800",
    minWidth: 22,
  },
  extractionStepText: {
    color: COLORS.onSurface,
    fontSize: 13,
    lineHeight: 19,
    flex: 1,
  },
  extractionShowMore: {
    marginTop: 4,
    alignSelf: "flex-start",
  },
  extractionShowMoreText: {
    color: COLORS.primary,
    fontSize: 13,
    fontWeight: "700",
  },
  extractionMacroRow: {
    flexDirection: "row",
    justifyContent: "space-around",
    gap: 8,
  },
  extractionMacroItem: {
    flex: 1,
    alignItems: "center",
    paddingVertical: 8,
    borderRadius: 8,
    backgroundColor: COLORS.surfaceContainer,
  },
  extractionMacroValue: {
    color: COLORS.primary,
    fontSize: 16,
    fontWeight: "900",
  },
  extractionMacroLabel: {
    color: COLORS.onSurfaceVariant,
    fontSize: 11,
    fontWeight: "700",
    marginTop: 2,
  },
  adminTabs: {
    gap: 8,
  },
  dataRow: {
    minHeight: 64,
    flexDirection: "row",
    alignItems: "center",
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    paddingHorizontal: 12,
    paddingVertical: 10,
    marginBottom: 10,
  },
  dataText: {
    flex: 1,
  },
  dataTitle: {
    color: COLORS.onSurface,
    fontSize: 15,
    fontWeight: "900",
    marginBottom: 2,
  },
  dataSubtitle: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    lineHeight: 17,
  },
  iconButton: {
    width: 34,
    height: 34,
    borderRadius: 17,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: COLORS.surfaceContainer,
    marginLeft: 8,
  },
  reviewItem: {
    borderRadius: 8,
    borderWidth: 1,
    borderColor: COLORS.outlineVariant,
    backgroundColor: COLORS.surfaceContainerLowest,
    padding: 12,
    marginBottom: 12,
  },
  reviewHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    marginBottom: 8,
  },
  reviewType: {
    color: COLORS.primary,
    fontSize: 13,
    fontWeight: "900",
  },
  reviewStatus: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    fontWeight: "800",
  },
  reviewContent: {
    color: COLORS.onSurfaceVariant,
    fontSize: 12,
    lineHeight: 17,
    marginBottom: 10,
  },
  detailTabPanel: {
    marginBottom: 20,
  },
  detailTabScroll: {
    marginBottom: 12,
  },
  detailTabContent: {
    flexDirection: "row",
    gap: 8,
    paddingRight: SPACING.lg,
  },
  hint: {
    color: COLORS.primary,
    fontSize: 13,
    lineHeight: 20,
    marginBottom: 10,
  },
  emptyText: {
    color: COLORS.onSurfaceVariant,
    fontSize: 14,
    lineHeight: 21,
  },
  loader: {
    marginVertical: 20,
  },
  savingOverlay: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
    paddingVertical: 14,
  },
  savingText: {
    color: COLORS.onSurfaceVariant,
    fontWeight: "700",
  },
});
